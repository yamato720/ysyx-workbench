...
#define SPMV_CUPERFLOW_MAP_ABI_FROZEN "cuperflow-map-multisegment-v4"
#endif
#ifndef SPMV_CUPERFLOW_ROW_BATCH_SIZE_FROZEN
#define SPMV_CUPERFLOW_ROW_BATCH_SIZE_FROZEN 8192
#endif
...
static_assert(kPcCount >= 1 && kPcCount <= 16 && kXLoadLanes == 8 && kWordsPerBeat == 8 &&
...
static_assert(std::string_view(SPMV_CUPERFLOW_MAP_ABI_FROZEN) ==
    "cuperflow-map-multisegment-v4",
    "Cuperflow RTL host 只接受 V0 预处理 ABI map v4");
static_assert(SPMV_CUPERFLOW_AXI_ADDR_WIDTH_FROZEN == 64 &&
    ...
    SPMV_CUPERFLOW_ROW_BATCH_SIZE_FROZEN > 0 &&
    SPMV_CUPERFLOW_ROW_BATCH_SIZE_FROZEN <= 8192 &&
    SPMV_CUPERFLOW_X_ELEMENT_WIDTH_FROZEN == 64,
    "Cuperflow RTL host 只支持当前 FP64/8192 配置");

struct TimingRequest {
  std::string dataset;
  cf::CuperflowTailPacking tailPacking = cf::CuperflowTailPacking::Pad3To4And1To2;
  std::string_view tailPackingName = "pad-3-1";
};

TimingRequest parseTimingRequest(const std::string& requested) {
  constexpr std::string_view kTailPrefix = "@tail=";
  const std::size_t suffix = requested.find(kTailPrefix);
  if (suffix == std::string::npos) {
    return TimingRequest{requested};
  }
  if (requested.find(kTailPrefix, suffix + kTailPrefix.size()) != std::string::npos ||
      suffix == 0 || suffix + kTailPrefix.size() == requested.size()) {
    throw std::invalid_argument(
        "Cuperflow tail 选择格式应为 <dataset>@tail=compact|pad-3-1|pad-all-4");
  }
  const std::string mode = requested.substr(suffix + kTailPrefix.size());
  if (mode == "compact") {
    return TimingRequest{requested.substr(0, suffix), cf::CuperflowTailPacking::Compact421,
                         "compact-4-2-1"};
  }
  if (mode == "pad-3-1") {
    return TimingRequest{requested.substr(0, suffix),
                         cf::CuperflowTailPacking::Pad3To4And1To2, "pad-3-1"};
  }
  if (mode == "pad-all-4") {
    return TimingRequest{requested.substr(0, suffix), cf::CuperflowTailPacking::PadAllTo4,
                         "pad-all-4"};
  }
  throw std::invalid_argument(
      "Cuperflow tail 模式必须是 compact、pad-3-1 或 pad-all-4");
}
...
std::array<HbmPort, kPcCount> hbmPorts(VSpmvCuperflowInputTop& dut) {
  return {{CUPERFLOW_HBM_PORT(0)
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 2
      , CUPERFLOW_HBM_PORT(1)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 3
      , CUPERFLOW_HBM_PORT(2)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 4
      , CUPERFLOW_HBM_PORT(3)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 5
      , CUPERFLOW_HBM_PORT(4)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 6
      , CUPERFLOW_HBM_PORT(5)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 7
      , CUPERFLOW_HBM_PORT(6)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 8
      , CUPERFLOW_HBM_PORT(7)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 9
      , CUPERFLOW_HBM_PORT(8)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 10
      , CUPERFLOW_HBM_PORT(9)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 11
      , CUPERFLOW_HBM_PORT(10)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 12
      , CUPERFLOW_HBM_PORT(11)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 13
      , CUPERFLOW_HBM_PORT(12)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 14
      , CUPERFLOW_HBM_PORT(13)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 15
      , CUPERFLOW_HBM_PORT(14)
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 16
...
/** 乘法-only construction 显式 drain ProductBeat；L2 standalone test 不复用此 host。 */
void driveProductReady(VSpmvCuperflowInputTop& dut) {
  dut.io_product_0_ready = 1;
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 2
  dut.io_product_1_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 3
  dut.io_product_2_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 4
  dut.io_product_3_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 5
  dut.io_product_4_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 6
  dut.io_product_5_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 7
  dut.io_product_6_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 8
  dut.io_product_7_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 9
  dut.io_product_8_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 10
  dut.io_product_9_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 11
  dut.io_product_10_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 12
  dut.io_product_11_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 13
  dut.io_product_12_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 14
  dut.io_product_13_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 15
  dut.io_product_14_ready = 1;
#endif
#if SPMV_CUPERFLOW_HBM_PC_COUNT_FROZEN >= 16
  dut.io_product_15_ready = 1;
#endif
}
      ...
      for (std::size_t batch = 0; batch < package.stats.batchCount; ++batch) {
        ...
        for (std::size_t lane = 0; lane < kWordsPerBeat; ++lane) {
          for (std::size_t beat = ranges[lane].first; beat < ranges[lane].second; ++beat) {
            if ((package.matrixEntryMasks[pc][beat] & (std::uint8_t{1} << lane)) == 0U) {
              continue;
            }
...
std::size_t sequentialXLoadCycles(const cf::CuperflowXRange& range) {
  if (range.markerCount != 0 || range.encodedWordCount != range.elementCount ||
    ...
    throw std::runtime_error(
      "Cuperflow map ABI v4 的紧凑 X payload 不允许地址 marker 或非顺序 payload");
  }
...
void writeTimingJson(std::ostream& output, const std::string& dataset,
  ...
  output << ",\"schedule\":\"per-pc-map-x-a\",\"pcCount\":" << kPcCount << ",\"xLoadLanes\":"
      << kXLoadLanes
...
int runCuperflowTiming(const std::string& requestedDataset) {
  const TimingRequest request = parseTimingRequest(
      requestedDataset.empty() ? "n512" : requestedDataset);
  const std::string& requested = request.dataset;
  if (requested == "--list") {
    std::cout << "Cuperflow RTL host: make -C accelerator-sim/spmv run "
              << "mainargs=<dataset[@tail=compact|pad-3-1|pad-all-4]"
              << "|synthetic-extreme-x-span|synthetic-three-islands-x-span>\n";
    return 0;
  }
  ...
  cf::CuperflowConfig encodingConfig;
  encodingConfig.hbmChannelCount = kPcCount;
  encodingConfig.rowBatchSize = SPMV_CUPERFLOW_ROW_BATCH_SIZE_FROZEN;
  encodingConfig.tailPacking = request.tailPacking;
  const cf::CuperflowPackage package = cf::encode(matrix, encodingConfig);
  ...
  if (package.config.hbmChannelCount != kPcCount || vectorPackage.channelXRanges.size() != kPcCount ||
    ...
    throw std::runtime_error("Cuperflow RTL host 的 package 通道数或 sliceGroup 非法");
  }
  ...
  bool sawX = false;
  bool sawA = false;
  std::uint64_t rtlAResponseBeats = 0;
  for (std::uint64_t localCycle = 0; localCycle < 20000000ULL; ++localCycle) {
    ...
    if (record.aR != 0) {
      rtlAResponseBeats += static_cast<std::uint64_t>(__builtin_popcount(record.aR));
      if (!sawA) {
  ...
  if (rtlChecksum != expectedChecksum) {
    ...
    message << "Cuperflow RTL productChecksum 与 golden 不一致: rtl=0x" << std::hex
            << rtlChecksum << " golden=0x" << expectedChecksum << std::dec
            << " error=" << static_cast<unsigned>(dut.io_error)
            << " a_response_beats=" << rtlAResponseBeats
            << " cycles=" << cycles.size();
  ...
  (void)xLoadedGroups;
  const std::string reportDataset = datasetPath.filename().string() + "-" +
      std::string(request.tailPackingName);
  const fs::path run = reportDirectory(reportDataset);
  writeTimingReport(run / "performance.html", reportDataset, works, cycles,
  ...
  writeTimingReport(run / "cuperflow-timing.html", reportDataset, works, cycles,
      totalABeats, package.stats.totalMatrixBeats, usefulSlots, physicalSlots, xSourceBeats,
      xWords, xMarkers, xWriteCyclesTotal, package.sliceGroupCount, waveCount,
      package.stats.batchCount, xLoadedGroups, rtlChecksum, expectedChecksum);
  updateLatest(run);
  ...
  std::cout << "[spmv-cuperflow-rtl] Verilator RTL simulation dataset=" << datasetPath
      << " tail_packing=" << request.tailPackingName << " order=map-x-a groups="
