...
inline constexpr std::string_view kVectorHtmlPrefix = R"CUPER(<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Cuper X 向量布局</title>
<style>
...
<header class="app-header">
  <div class="identity">
    <div class="identity-main"><h1>Cuper X 向量布局</h1><span class="dataset" id="dataset"></span></div>
    <nav class="report-tabs" aria-label="编码报告"><a id="matrixReport">A Matrix</a><span class="active">X Vector</span></nav>
  </div>
...
<main class="workspace">
  <div class="flow-band">
    ...
    <div class="flow-node encoded"><strong>X broadcast → local_X</strong><span id="coreFlow"></span></div><div class="flow-arrow">→</div>
  ...
  <section id="batchView" hidden>
    ...
    <div class="toolbar">
      <div class="segmented" id="cellMode" aria-label="元素显示模式"><button type="button" data-mode="value" class="active">数值</button><button type="button" data-mode="column">列号</button><button type="button" data-mode="bits">FP32 Bits</button></div>
...
inline constexpr std::string_view kVectorHtmlSuffix = R"CUPER(;
const I={seq:0,batch:1,beat:2,batchBeat:3,lane:4,column:5,localColumn:6,padding:7,source:8,fp32:9,bits:10,bank:11};
...
function renderElement(){
  ...
  encoded.append(make('strong','mono',e[I.fp32]),make('span','',e[I.bits]+' · FP32'));
  ...
  if(!e[I.padding]){
    for(let replica=0;replica<report.config.replicas;replica++){
      ...
      card.append(
        ...
        make('span','','Bank '+e[I.bank]+' · Core 0–'+(report.config.coreCount-1)),
      );
  ...
  position.append(make('h3','','HBM 坐标'),detailTable([
    ...
    ['float_v16 lane',e[I.lane]],
    ['FP32 bits',e[I.bits],'mono'],
  ]));
  layout.append(make('h3','','Core 本地映射'),detailTable([
    ['广播范围','Core 0–'+(report.config.coreCount-1)],
...
E.dataset.textContent=report.dataset;E.matrixReport.href=report.matrixReport;E.packageMeta.textContent=number(report.stats.validElements)+' columns · b.txt FP64 → HBM FP32 float_v16';E.sourcePath.textContent=report.source;E.sourcePath.title=report.source;E.mapFlow.textContent='Batch_num / Row_num / Iteration_num / Column_num';E.coreFlow.textContent=report.config.coreCount+' cores · 逐级转发并写 '+report.config.replicas+' 份 local_X';E.bankFlow.textContent='读取 SpElement_list_ptr[i+1] · '+report.config.partitionFactor+' cyclic banks';E.computeFlow.textContent='消费当前 batch 的 Matrix_A_Stream 并乘加';
renderPackage();
</script>
</body>
</html>)CUPER";
