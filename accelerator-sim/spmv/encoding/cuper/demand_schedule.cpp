...
std::vector<std::size_t> pageArrivalCycles(const std::vector<std::size_t>& pageOrder,
  ...
  for (std::size_t page : pageOrder) {
    if (page >= pageCycles.size() || pageCycles[page] >
      ...
      throw std::overflow_error("Cuper X page 调度周期溢出");
    }
...
CuperDemandChannelTiming simulateChannel(const ChannelDependencies& beats,
                                          const std::vector<std::size_t>& pageArrivals,
                                          std::uint64_t xLoadCycles) {
  CuperDemandChannelTiming timing;
  ...
  for (const PageDependencies& dependencies : beats) {
    ...
    if (issueCycle == std::numeric_limits<std::uint64_t>::max()) {
      throw std::overflow_error("Cuper A 发射周期溢出");
    }
...
void populateBatchSummary(CuperDemandBatchPlan* plan) {
  auto summarize = [plan](bool planned) {
    CuperDemandBatchTiming timing;
    bool first = true;
    for (const CuperDemandChannelPlan& channel : plan->channels) {
      const CuperDemandChannelTiming& source = planned ? channel.planned : channel.baseline;
      if (!source.active) {
        continue;
      }
...
void writeTimingJson(std::ostream& output, const CuperDemandChannelTiming& timing) {
...
void writeBatchTimingJson(std::ostream& output, const CuperDemandBatchTiming& timing) {
...
CuperDemandSchedule planXPageSchedule(const CuperPackage& package,
                                       const CuperDemandScheduleConfig& requestedConfig) {
  if (package.config.sliceSize == 0 || package.stats.batchCount == 0 ||
    ...
    throw std::invalid_argument("Cuper X page 调度收到不完整的 matrix package");
  }
  CuperDemandSchedule schedule;
  ...
  if (schedule.config.pageElements == 0 ||
    ...
    throw std::invalid_argument("Cuper X page 调度的 page 或输入带宽必须为正数");
  }

  const std::size_t batchWidth = columnsPerBatch(package.config);
  for (std::size_t batch = 0; batch < package.stats.batchCount; ++batch) {
    const std::size_t firstColumn = batch * batchWidth;
    if (firstColumn >= package.columns) {
      throw std::invalid_argument("Cuper X page 调度发现空 column batch");
    }
    const std::size_t batchColumns = std::min(batchWidth, package.columns - firstColumn);
    const std::size_t pageCount = divideRoundedUp(batchColumns, schedule.config.pageElements);
    ...
    for (std::size_t channel = 0; channel < package.config.hbmChannelCount; ++channel) {
      ...
      if (pointers.size() != package.stats.batchCount + 1U ||
          pointers[batch] > pointers[batch + 1U] ||
          pointers[batch + 1U] > package.matrixChannels[channel].size()) {
        throw std::invalid_argument("Cuper X page 调度发现非法 channel batch pointer");
      }
      const std::size_t begin = pointers[batch];
      const std::size_t end = pointers[batch + 1U];
      dependencies[channel].reserve(end - begin);
      for (std::size_t beat = begin; beat < end; ++beat) {
        std::vector<bool> required(pageCount, false);
        for (std::size_t lane = 0; lane < kLanesPerBeat; ++lane) {
          const DecodedCuperSlot slot = decodeSlot(package.matrixChannels[channel][beat][lane]);
          const std::size_t page = slot.localColumn / schedule.config.pageElements;
          if (page >= pageCount) {
            throw std::invalid_argument("Cuper A slot 的 local column 不属于当前 X batch");
          }
          required[page] = true;
        }
        PageDependencies beatDependencies;
        for (std::size_t page = 0; page < pageCount; ++page) {
          if (required[page]) {
            beatDependencies.push_back(page);
          }
        }
        dependencies[channel].push_back(std::move(beatDependencies));
      }
    }
    ...
    for (std::size_t channel = 0; channel < dependencies.size(); ++channel) {
      ...
      for (std::size_t page : firstBeat) {
        if (pageCycles[page] > std::numeric_limits<std::size_t>::max() - candidateCycles) {
          throw std::overflow_error("Cuper X 首发 page 调度周期溢出");
        }
    ...
    if (seedChannel == dependencies.size()) {
      throw std::logic_error("Cuper X page 调度未找到首个 A beat");
    }
    ...
    while (pageOrder.size() < pageCount) {
      ...
      if (selected == pageCount) {
        throw std::logic_error("Cuper X page 调度未能选择下一个 page");
      }
    ...
    if (plannedXLoadCycles != xLoadCycles) {
      throw std::logic_error("Cuper X page 重排意外改变了 X 总装载周期");
    }

    CuperDemandBatchPlan plan;
    plan.batch = batch;
    plan.columns = batchColumns;
    ...
    for (std::size_t channel = 0; channel < dependencies.size(); ++channel) {
      CuperDemandChannelPlan channelPlan;
...
CuperPackage remapLocalColumnsForXPageSchedule(
  ...
  if (schedule.config.pageElements == 0 ||
      ...
      package.matrixChannels.size() != package.config.hbmChannelCount ||
      package.channelBatchPointers.size() != package.config.hbmChannelCount) {
    throw std::invalid_argument("Cuper X page 重排收到不完整的 package 或 schedule");
  }

  CuperPackage remapped = package;
  constexpr std::uint64_t columnFieldMask = ((std::uint64_t{1} << kColumnBits) - 1U) << 51U;
  const std::size_t batchWidth = columnsPerBatch(package.config);
  for (std::size_t batch = 0; batch < package.stats.batchCount; ++batch) {
    const CuperDemandBatchPlan& plan = schedule.batches[batch];
    const std::size_t firstColumn = batch * batchWidth;
    const std::size_t batchColumns = std::min(batchWidth, package.columns - firstColumn);
    const std::size_t expectedPageCount = divideRoundedUp(batchColumns,
                                                          schedule.config.pageElements);
    if (plan.batch != batch || plan.columns != batchColumns ||
        plan.pageCount != expectedPageCount || plan.pageOrder.size() != expectedPageCount) {
      throw std::invalid_argument("Cuper X page schedule 的 batch 描述与 package 不一致");
    }

    std::vector<std::size_t> physicalPages(expectedPageCount, expectedPageCount);
    for (std::size_t physicalPage = 0; physicalPage < plan.pageOrder.size(); ++physicalPage) {
      ...
      if (logicalPage >= expectedPageCount || physicalPages[logicalPage] != expectedPageCount) {
        throw std::invalid_argument("Cuper X page schedule 不是一个 page 排列");
      }
    ...
    for (std::size_t channel = 0; channel < package.config.hbmChannelCount; ++channel) {
      ...
      if (pointers.size() != package.stats.batchCount + 1U ||
          pointers[batch] > pointers[batch + 1U] ||
          pointers[batch + 1U] > remapped.matrixChannels[channel].size()) {
        throw std::invalid_argument("Cuper X page 重排发现非法 channel batch pointer");
      }
      for (std::size_t beat = pointers[batch]; beat < pointers[batch + 1U]; ++beat) {
        for (std::size_t lane = 0; lane < kLanesPerBeat; ++lane) {
          const std::uint64_t slot = remapped.matrixChannels[channel][beat][lane];
          const std::size_t localColumn = static_cast<std::size_t>(
              (slot >> 51U) & ((std::uint64_t{1} << kColumnBits) - 1U));
          const std::size_t logicalPage = localColumn / schedule.config.pageElements;
          if (logicalPage >= expectedPageCount) {
            throw std::invalid_argument("Cuper A slot 的 local column 不属于当前 X batch");
          }
          const std::size_t remappedColumn = physicalPages[logicalPage] *
              schedule.config.pageElements + localColumn % schedule.config.pageElements;
          if (remappedColumn >= (std::size_t{1} << kColumnBits)) {
            throw std::overflow_error("Cuper X page 重排后的 local column 超过 slot 位域");
          }
          remapped.matrixChannels[channel][beat][lane] =
              (slot & ~columnFieldMask) | (static_cast<std::uint64_t>(remappedColumn) << 51U);
        }
      }
    }
  }
...
void writeDemandScheduleJson(std::ostream& output, const CuperDemandSchedule& schedule,
  ...
  output << "{\"format\":\"cuper-x-page-demand-v1\",\"dataset\":";
  ...
  for (std::size_t index = 0; index < schedule.batches.size(); ++index) {
    const CuperDemandBatchPlan& batch = schedule.batches[index];
    ...
    for (std::size_t channel = 0; channel < batch.channels.size(); ++channel) {
      const CuperDemandChannelPlan& plan = batch.channels[channel];
