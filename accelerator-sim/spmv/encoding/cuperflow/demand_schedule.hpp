...
/** 离线 X page 调度器使用的输入带宽模型。
  ...
  * 机会，不会修改 Cuperflow A package 或 RTL ABI。
  */
struct CuperflowDemandScheduleConfig {
  std::size_t pageElements = 0;
  std::size_t xElementsPerCycle = 8;
};

struct CuperflowDemandChannelTiming {
  bool active = false;
...
struct CuperflowDemandChannelPlan {
  std::size_t channel = 0;
  std::size_t aBeats = 0;
  CuperflowDemandChannelTiming baseline;
  CuperflowDemandChannelTiming planned;
};

struct CuperflowDemandBatchTiming {
  std::uint64_t firstIssueCycle = 0;
  std::uint64_t lastIssueCycle = 0;
  std::uint64_t issuedBeforeXComplete = 0;
...
struct CuperflowDemandBatchPlan {
  std::size_t batch = 0;
  ...
  CuperflowDemandBatchTiming baseline;
  CuperflowDemandBatchTiming planned;
  std::vector<CuperflowDemandChannelPlan> channels;
};

struct CuperflowDemandSchedule {
  CuperflowDemandScheduleConfig config;
  /** 与 matrix package 一致的逻辑 slice group 几何。 */
  std::size_t sliceGroupSize = 0;
  std::size_t sliceGroupCount = 0;
  /** 与 matrix/vector package 一致的 HBM-owned X range 映射。 */
  std::vector<std::size_t> sliceGroupChannels;
  std::vector<std::vector<std::size_t>> channelSliceGroups;
  std::vector<CuperflowDemandBatchPlan> batches;
};
 ...
 * 任意交换 A beat，因而不破坏现有 Cuperflow RAW 排程的顺序约束。
 */
CuperflowDemandSchedule planXPageSchedule(
    const CuperflowPackage& package,
    const CuperflowDemandScheduleConfig& config = {});

/**
 * 将每个 batch 的 A slot group-local column 改写为 schedule 对应的物理 X page 位置。
 *
