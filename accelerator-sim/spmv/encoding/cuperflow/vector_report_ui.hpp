...
function renderPackage(){show('packageView');const physicalSlots=report.stats.encodedPayloadBeats*report.config.lanesPerBeat;renderStats(E.packageStats,[['输入元素',number(report.stats.validElements),'source'],['编码模式',report.config.flexibleX?'多段紧凑 X':'连续满载 X','encoded'],['A 实际需求列',number(report.stats.demandedElements),'source'],['X 负载利用率',percentage(report.stats.demandedElements,report.stats.encodedValueCount),'encoded'],['有效 value lane',percentage(report.stats.encodedValueCount,physicalSlots),'encoded'],['顺序 stream beat',number(report.stats.encodedPayloadBeats),'encoded'],['HBM-owned ranges',number(report.stats.rangeCount),'encoded'],['连续 X 段',number(report.stats.segmentCount),'encoded'],['最大紧凑 payload',number(report.stats.maximumRangeElements)+' elements'],['Address markers',number(report.stats.markerCount),'encoded'],['HBM allocation',bytes(report.stats.allocatedBytes)],['local_X copies / HBM',number(report.config.replicas),'bank']]);E.batchGrid.replaceChildren();for(let batch=0;batch<report.stats.batchCount;batch++){const bounds=batchBounds(batch),valid=Math.max(0,bounds.lastColumn-bounds.firstColumn+1),padding=bounds.beats*report.config.lanesPerBeat-valid,card=make('button','batch-card');card.type='button';const head=make('div','card-head'),title=make('div');title.append(make('div','card-title','Batch '+batch),make('div','card-range','C '+number(bounds.firstColumn)+' – '+number(bounds.lastColumn)));head.append(title,make('div','card-count',number(valid)+' values'));const strip=make('div','packet-strip'),validBar=make('span','packet-valid'),paddingBar=make('span','packet-padding');validBar.style.flex=String(valid);paddingBar.style.flex=String(padding);strip.append(validBar,paddingBar);card.append(head,strip,make('div','card-meta',number(bounds.beats)+' FP64 beats · '+number(padding)+' lane padding'));card.addEventListener('click',()=>{state.batch=batch;state.page=0;renderBatch()});E.batchGrid.appendChild(card)}}
...
function renderElement(){
  ...
  const owner=report.channelXRanges.findIndex(ranges=>ranges.some(range=>range[5].some(segment=>e[I.column]>=range[1]+segment[0]&&e[I.column]<range[1]+segment[0]+segment[1])));
...
E.dataset.textContent=report.dataset;E.matrixReport.href=report.matrixReport;E.timingReport.href=report.timingReport;E.packageMeta.textContent=number(report.stats.validElements)+' columns · '+number(report.stats.rangeCount)+' 个 HBM-owned X range · FP64 → FP64 · '+(report.config.flexibleX?'多段紧凑 X':'连续满载 X')+' · '+number(report.stats.segmentCount)+' segments';E.sourcePath.textContent=report.source;E.sourcePath.title=report.source;E.mapFlow.textContent='map: segment start/count / payload / A range';E.coreFlow.textContent=report.config.coreCount+' HBM · 多段 FP64 X 按段顺序写入 '+report.config.replicas+' 份 local_X';E.bankFlow.textContent='A segmentId 前缀 + localColumn - start · '+report.config.partitionFactor+' cyclic banks';E.computeFlow.textContent='消费当前 HBM range 的 Matrix_A_Stream 并乘加';
renderPackage();
</script>
</body>
</html>)CUPERFLOW";
