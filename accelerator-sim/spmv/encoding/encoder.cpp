...
EncodedVector encodeVector(const std::vector<double>& input, const EncodingOptions& options) {
  switch (options.format) {
    case EncodingFormat::Cuper:
...
void writeVectorHtmlReport(const std::filesystem::path& path, const EncodedVector& encoded,
  ...
  std::filesystem::create_directories(path.parent_path(), error);
  if (error) {
    throw std::runtime_error("无法创建 SpMV X 编码报告目录 " +
        path.parent_path().string() + ": " + error.message());
  }
  ...
  std::ofstream output(temporary);
  if (!output) {
    throw std::runtime_error("无法打开 SpMV X 编码报告: " + temporary.string());
  }
  writeVectorHtmlReport(output, encoded, metadata);
  output.close();
  if (!output) {
    std::filesystem::remove(temporary);
    throw std::runtime_error("无法写入 SpMV X 编码报告: " + temporary.string());
  }

  std::filesystem::rename(temporary, path, error);
  if (error) {
    std::filesystem::remove(temporary);
    throw std::runtime_error("无法发布 SpMV X 编码报告 " + path.string() + ": " +
        error.message());
  }
}
