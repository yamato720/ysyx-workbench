...
static int write_document(
  ...
  static const char *prefix =
      ...
      "--if:#16697a;--id:#8f5d18;--ex:#8c2f39;--mem:#386641;--wb:#5a3d8a;--handoff:#7a8794;--redirect:#9a5b13;--commit:#536471;--queue:#b25d19;--order:#486581;--error:#b42318}*{box-sizing:border-box}"
      ...
      ".stage.handoff,.stage.redirect,.stage.commit,.stage.queue,.stage.order,.stage.error{border:1px dashed rgba(255,255,255,.7);padding:3px 4px}.stage.handoff{background:var(--handoff)}.stage.redirect{background:var(--redirect)}.stage.commit{background:var(--commit)}.stage.queue{background:var(--queue)}.stage.order{background:var(--order)}.stage.error{background:var(--error)}"
      ...
      "<div class=\"legend\"><span class=\"key\" style=\"--c:var(--if)\">IF</span><span class=\"key\" style=\"--c:var(--id)\">IF/ID</span><span class=\"key\" style=\"--c:var(--ex)\">EX</span><span class=\"key\" style=\"--c:var(--mem)\">MEM</span><span class=\"key\" style=\"--c:var(--wb)\">WB</span><span class=\"key\" style=\"--c:var(--handoff)\">边界等待</span><span class=\"key\" style=\"--c:var(--redirect)\">REDIRECT</span><span class=\"key\" style=\"--c:var(--commit)\">COMMIT</span><span class=\"key\" style=\"--c:var(--queue)\">QUEUE</span><span class=\"key\" style=\"--c:var(--order)\">ORDER</span></div>"
  ...
  // 非访存项的 MEM service 为零，却可能在 orderQueue 中等待较老访存；时间线用 ORDER
  // 显示这段按序等待，避免把它误认为缓存或下游总线停顿。
  static const char *suffix =
      "]};const names=['IF','IF/ID','EX','MEM','WB'];const colors=['var(--if)','var(--id)','var(--ex)','var(--mem)','var(--wb)'];const gapNames=['','IF/ID','ID/EX','EX/MEM','MEM/WB'];const nextBySequence=new Map(trace.records.map((record,index)=>[record.n,trace.records[index+1]||null]));"
      ...
      "const total=trace.captured+trace.dropped;document.querySelector('#summary').textContent=`${trace.label} · 提交 ${total.toLocaleString()} 条 · 记录 ${trace.captured.toLocaleString()} 条 · 阶段起点 ${trace.absolute_records.toLocaleString()} 条使用硬件时间戳 · 背压周期 IF starvation/IF/ID/EX/MEM=${trace.stalls.slice(0,4).join('/')} · flush=${trace.stalls[4]} · MEM 统计=${trace.memory_statistics_mode}`;"
      ...
      "function controlTransfer(record,next){const inst=Number.parseInt(record.inst,16)>>>0,opcode=inst&127;if(opcode===103||opcode===111)return true;if(!next)return false;const length=(inst&3)===3?4:2,fallthrough=Number.parseInt(record.pc,16)+length;return Number.parseInt(next.pc,16)!==fallthrough}function timelineBlocks(record,next){const blocks=[];let previousEnd=null,previousName='开始',memory=record.m;for(let i=0;i<record.d.length;i++){const duration=record.d[i];if(!duration)continue;const start=record.s[i],end=start+duration-1;if(i===3&&memory&&memory.queue>0&&trace.memory_statistics_mode!=='ServiceOnly'){const queueStart=Math.max(memory.queueStart,previousEnd===null?memory.queueStart:previousEnd),queueEnd=start-1;if(queueEnd>=queueStart){blocks.push({kind:'queue',name:'QUEUE',start:queueStart,end:queueEnd,duration:queueEnd-queueStart+1,from:previousName,to:'MEM'});previousEnd=start;previousName='QUEUE';}}if(previousEnd!==null&&start>previousEnd){const name=gapNames[i]||'BACKPRESSURE';blocks.push({kind:'handoff',name,start:previousEnd,end:start-1,duration:start-previousEnd,from:previousName,to:names[i]});}else if(previousEnd!==null&&start<previousEnd){blocks.push({kind:'error',name:'TIMING OVERLAP',start,end:previousEnd-1,duration:previousEnd-start,from:previousName,to:names[i]});}blocks.push({kind:'stage',name:names[i],stage:i,start,end,duration});previousEnd=Math.max(previousEnd===null?0:previousEnd,start+duration);previousName=names[i];}if(previousEnd!==null&&record.commit>=previousEnd){blocks.push({kind:'commit',name:'COMMIT',start:previousEnd,end:record.commit,duration:record.commit-previousEnd+1,from:previousName,to:'COMMIT'});}if(next&&next.s[0]>record.commit+1){const start=record.commit+1,end=next.s[0]-1,redirect=controlTransfer(record,next);blocks.push({kind:redirect?'redirect':'handoff',name:redirect?'REDIRECT':'FRONTEND WAIT',start,end,duration:end-start+1,from:'COMMIT',to:'IF'});}return blocks}"
