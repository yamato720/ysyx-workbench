...
控制器、CSR 或完整 GPR 快照；普通 v11 终端则显式保持 `FPGA_RUNTIME_SDB=1`。ZCU102 使用 `npc-fpga-runtime-v7`。M 扩展继续由 Xilinx
...
退出码和提交 PC；EBREAK 只保留正常同步 breakpoint trap 状态；completion 不是 debug halt，也不是 guest 外部中断。
