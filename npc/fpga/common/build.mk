...
FPGA_RELEASE_CONSTRUCTIONS ?= $(FPGA_COMMON_DIR)/releases/v0.2.0-fpga-sdb/constructions.env
RELEASE_CONSTRUCTION ?=
FPGA_BOARD_RTL_DIR := $(FPGA_BOARD_DIR)/rtl
FPGA_IP_GENERATOR_ROOT := $(abspath $(FPGA_ROOT)/../fpga-ip-generator)
FPGA_IP_ADAPTER_DIR := $(FPGA_IP_GENERATOR_ROOT)/common/compute/source/sv
FPGA_IP_GENERATOR := $(FPGA_IP_GENERATOR_ROOT)/common/compute/source/tcl/create-arithmetic-ip.tcl
FPGA_RTL_DIR := $(FPGA_WORK_DIR)/rtl
FPGA_IP_DIR := $(FPGA_WORK_DIR)/ip-generated
...
FPGA_MANIFEST_VALUES = \
		...
		BOARD_PART=$(FPGA_BOARD_PART) CLOCK_MHZ=$(FPGA_CLOCK_MHZ) PLATFORM_CLOCK_MHZ=$(FPGA_PLATFORM_CLOCK_MHZ) VIVADO_VERSION=$(FPGA_VIVADO_VERSION) \
		...
		XLEN=$(NPC_XLEN) AXI_MEMORY_DATA_WIDTH=$(AXI_MEMORY_DATA_WIDTH) M=$(NPC_M) F=$(NPC_F) D=$(NPC_D) ZICSR=$(ZICSR) \
		MEMORY_BASE=$(FPGA_MEMORY_BASE) MEMORY_HOST_BASE=$(FPGA_MEMORY_HOST_BASE) MEMORY_SIZE=$(FPGA_MEMORY_SIZE) \
		FPGA_RUNTIME_SDB=$(FPGA_RUNTIME_SDB) FPGA_RUNTIME_TRACE=$(FPGA_RUNTIME_TRACE) FPGA_TRACE_HBM_BANK=$(FPGA_TRACE_HBM_BANK) \
		FPGA_TRACE_BUFFER_BYTES=$(FPGA_TRACE_BUFFER_BYTES) FPGA_TRACE_MAX_RECORDS=$(FPGA_TRACE_MAX_RECORDS) \
		FPGA_TRACE_CACHE_RECORDS=$(FPGA_TRACE_CACHE_RECORDS) FPGA_TRACE_FORMAT=$(FPGA_TRACE_FORMAT) \
		FPGA_TRACE_RECORD_BYTES=$(FPGA_TRACE_RECORD_BYTES) FPGA_TRACE_DATA_WIDTH=$(FPGA_TRACE_DATA_WIDTH) \
		FPGA_TRACE_BURST_RECORDS=$(FPGA_TRACE_BURST_RECORDS) \
	MUL_LATENCY=$(NPC_MUL_CYCLES) MUL_II=$(NPC_MUL_II) DIV_LATENCY=$(NPC_DIV_CYCLES) DIV_II=$(NPC_DIV_II) \
	DIV_IP_LATENCY=$(FPGA_DIV_IP_CYCLES) DIV_ADAPTER_LATENCY=$(FPGA_DIV_ADAPTER_CYCLES) DIV_NON_BLOCKING=$(FPGA_DIVIDER_NON_BLOCKING) \
...
FPGA_ELAB_MANIFEST_VALUES = \
	CONFIG_FQCN=$(FPGA_SCALA_CONFIG) FPGA_BOARD=$(FPGA_CONFIG_NAME) FPGA_CLOCK_MHZ=$(FPGA_CLOCK_MHZ) FPGA_PLATFORM_CLOCK_MHZ=$(FPGA_PLATFORM_CLOCK_MHZ) \
	...
	FPGA_TRACE_RECORD_BYTES=$(FPGA_TRACE_RECORD_BYTES) FPGA_TRACE_DATA_WIDTH=$(FPGA_TRACE_DATA_WIDTH) \
	FPGA_TRACE_BURST_RECORDS=$(FPGA_TRACE_BURST_RECORDS) \
	FPGA_IP_ATTACHMENT=$(FPGA_IP_ATTACHMENT) \
	FPGA_DIV_IP_CYCLES=$(FPGA_DIV_IP_CYCLES) FPGA_DIV_ADAPTER_CYCLES=$(FPGA_DIV_ADAPTER_CYCLES) \
	FPGA_DIVIDER_NON_BLOCKING=$(FPGA_DIVIDER_NON_BLOCKING) \
	NPC_TARGET=$(NPC_TARGET) NPC_XLEN=$(NPC_XLEN) NPC_AXI_MEMORY_DATA_WIDTH=$(AXI_MEMORY_DATA_WIDTH) NPC_PIPELINE=$(NPC_PIPELINE) NPC_ZICSR=$(ZICSR) NPC_ZIFENCEI=$(ZIFENCEI) \
	NPC_INTERLOCK=$(NPC_INTERLOCK) NPC_ID_FWD=$(NPC_ID_FWD) NPC_EX_FWD=$(NPC_EX_FWD) \
	NPC_M=$(NPC_M) NPC_F=$(NPC_F) NPC_D=$(NPC_D) \
	ICACHE_ENABLED=$(ICACHE_ENABLED) ICACHE_CAPACITY_BYTES=$(ICACHE_CAPACITY_BYTES) ICACHE_LINE_BYTES=$(ICACHE_LINE_BYTES) \
	ICACHE_MAPPING=$(ICACHE_MAPPING) ICACHE_WAYS=$(ICACHE_WAYS) ICACHE_SETS=$(ICACHE_SETS) \
	ICACHE_REPLACEMENT=$(ICACHE_REPLACEMENT) ICACHE_READ_MISS=$(ICACHE_READ_MISS) \
	ICACHE_WRITE_POLICY=$(ICACHE_WRITE_POLICY) ICACHE_WRITE_MISS=$(ICACHE_WRITE_MISS) ICACHE_STORAGE=$(ICACHE_STORAGE) \
	DCACHE_ENABLED=$(DCACHE_ENABLED) DCACHE_CAPACITY_BYTES=$(DCACHE_CAPACITY_BYTES) DCACHE_LINE_BYTES=$(DCACHE_LINE_BYTES) \
	DCACHE_MAPPING=$(DCACHE_MAPPING) DCACHE_WAYS=$(DCACHE_WAYS) DCACHE_SETS=$(DCACHE_SETS) \
	DCACHE_REPLACEMENT=$(DCACHE_REPLACEMENT) DCACHE_READ_MISS=$(DCACHE_READ_MISS) \
	DCACHE_WRITE_POLICY=$(DCACHE_WRITE_POLICY) DCACHE_WRITE_MISS=$(DCACHE_WRITE_MISS) DCACHE_STORAGE=$(DCACHE_STORAGE) \
	L2CACHE_ENABLED=$(L2CACHE_ENABLED) L2CACHE_CAPACITY_BYTES=$(L2CACHE_CAPACITY_BYTES) L2CACHE_LINE_BYTES=$(L2CACHE_LINE_BYTES) \
	L2CACHE_MAPPING=$(L2CACHE_MAPPING) L2CACHE_WAYS=$(L2CACHE_WAYS) L2CACHE_SETS=$(L2CACHE_SETS) \
	L2CACHE_REPLACEMENT=$(L2CACHE_REPLACEMENT) L2CACHE_READ_MISS=$(L2CACHE_READ_MISS) \
	L2CACHE_WRITE_POLICY=$(L2CACHE_WRITE_POLICY) L2CACHE_WRITE_MISS=$(L2CACHE_WRITE_MISS) L2CACHE_STORAGE=$(L2CACHE_STORAGE) \
	INSTRUCTION_BUFFER_ENABLED=$(INSTRUCTION_BUFFER_ENABLED) INSTRUCTION_BUFFER_ENTRIES=$(INSTRUCTION_BUFFER_ENTRIES) \
	NPC_ARITH_BACKEND=$(NPC_ARITH_BACKEND) NPC_ARITH_OUTPUT_FIFO=$(NPC_ARITH_OUTPUT_FIFO) \
...
FPGA_IP_ACTUAL_VALUES = \
	BOARD=$(FPGA_BOARD) PART=$(FPGA_PART) XLEN=$(NPC_XLEN) \
	MUL_LATENCY=$(NPC_MUL_CYCLES) MUL_II=$(NPC_MUL_II) \
	DIV_IP_LATENCY=$(FPGA_DIV_IP_CYCLES) DIV_II=$(NPC_DIV_II) DIV_NON_BLOCKING=$(FPGA_DIVIDER_NON_BLOCKING)
...
fpga-config:
	@printf '%s\n' \
		...
		'clock_mhz=$(FPGA_CLOCK_MHZ)' 'platform_clock_mhz=$(FPGA_PLATFORM_CLOCK_MHZ)' 'timing_wns_min_ns=$(FPGA_TIMING_WNS_MIN_NS)' \
		'notification_mode=$(FPGA_NOTIFICATION_MODE)' \
		'runtime_sdb=$(FPGA_RUNTIME_SDB)' 'runtime_trace=$(FPGA_RUNTIME_TRACE)' 'trace_hbm_bank=$(FPGA_TRACE_HBM_BANK)' \
		'trace_buffer_bytes=$(FPGA_TRACE_BUFFER_BYTES)' 'trace_max_records=$(FPGA_TRACE_MAX_RECORDS)' \
		'trace_cache_records=$(FPGA_TRACE_CACHE_RECORDS)' 'trace_format=$(FPGA_TRACE_FORMAT)' \
		'trace_record_bytes=$(FPGA_TRACE_RECORD_BYTES)' 'trace_data_width=$(FPGA_TRACE_DATA_WIDTH)' \
		'trace_burst_records=$(FPGA_TRACE_BURST_RECORDS)' \
		...
		'soc=$(FPGA_SOC)' 'target=$(NPC_TARGET)' 'xlen=$(NPC_XLEN)' 'axi_memory_data_width=$(AXI_MEMORY_DATA_WIDTH)' \
...
fpga-check:
	...
	@test "$(FPGA_DIVIDER_NON_BLOCKING)" = 0 || test "$(FPGA_DIVIDER_NON_BLOCKING)" = 1 || { \
		echo 'FPGA_DIVIDER_NON_BLOCKING must be 0 or 1' >&2; exit 2; }
...
$(FPGA_ELAB_DONE): FORCE fpga-check
	...
	if test "$(FPGA_TOOL_DRY_RUN)" = 1; then \
		...
		else echo "cd chisel/ysyxSoC && mill -i ysyxsoc.runMain npc.ElaborateFPGA --target-dir $(FPGA_RTL_DIR)"; fi; \
		touch "$@"; exit 0; \
	fi; \
	...
	if test -n "$(FPGA_SOC)"; then \
		...
		mill -i ysyxsoc.runMain npc.ElaborateFPGA --target-dir "$(FPGA_RTL_DIR)"; fi; \
...
$(FPGA_IP_DONE): FORCE fpga-check
	...
	vivado -mode batch -nojournal -nolog -source "$(FPGA_IP_GENERATOR)" -tclargs \
		"$(FPGA_IP_DIR)" "$(FPGA_IP_ACTUAL_MANIFEST)" "$(FPGA_IP_LOG_DIR)" "$(CONSTRUCTION_PROFILE)"; \
...
$(FPGA_SYNTH_DONE): FORCE $(FPGA_SYNTH_PREREQUISITES)
	...
	if test "$(FPGA_TYPE)" = zynqmp; then \
		...
		vivado -mode batch -nojournal -nolog -source "$(FPGA_SYNTH_TCL)" -tclargs \
			"$(FPGA_SYNTH_DIR)/project" "$(FPGA_PART)" "$(FPGA_SYNTH_TOP)" "$(FPGA_SYNTH_SOURCE_MANIFEST)" "$(FPGA_XO)" "$(NPC_XLEN)" "$(AXI_MEMORY_DATA_WIDTH)" \
			"$(FPGA_VIVADO_SYNTH_JOBS)" "$(FPGA_PLATFORM_CLOCK_MHZ)" "$(FPGA_CLOCK_MHZ)" "$(FPGA_RUNTIME_TRACE)"; \
	fi; \
...
$(FPGA_LINK_DONE): FORCE $(FPGA_LINK_PREREQUISITES)
	...
	if test "$(FPGA_TYPE)" = alveo; then \
		...
		{ cat "$(FPGA_LINK_CONFIG)"; \
			if test "$(FPGA_RUNTIME_TRACE)" = 1; then \
				printf 'sp=NpcFpgaKernel_1.m_axi_trace:HBM[%s]\n' "$(FPGA_TRACE_HBM_BANK)"; \
			fi; \
	...
	if test "$(FPGA_TYPE)" = zynqmp; then \
		...
		if test "$(FPGA_CONFIG_NAME)" = u55c; then "$(FPGA_PLATFORM_CLOCK_VERIFIER)" "$(FPGA_XCLBIN)" "$(FPGA_PLATFORM_CLOCK_MHZ)"; fi; \
...
fpga-config-test:
	@"$(FPGA_COMMON_DIR)/tests/config-regression.sh" "$(CURDIR)"

fpga-rtl-test:
	@"$(FPGA_COMMON_DIR)/tests/run-fpga-rtl-test.sh" "$(CURDIR)"

fpga-release-test:
	@"$(FPGA_COMMON_DIR)/tests/release-regression.sh" "$(CURDIR)"
