...
FPGA_MANIFEST_VALUES = \
		...
		FPGA_RUNTIME_TRACE=$(FPGA_RUNTIME_TRACE) FPGA_TRACE_HBM_BANK=$(FPGA_TRACE_HBM_BANK) \
		FPGA_TRACE_BUFFER_BYTES=$(FPGA_TRACE_BUFFER_BYTES) FPGA_TRACE_MAX_RECORDS=$(FPGA_TRACE_MAX_RECORDS) \
		FPGA_TRACE_CACHE_RECORDS=$(FPGA_TRACE_CACHE_RECORDS) \
...
fpga-config:
	@printf '%s\n' \
		...
		'runtime_trace=$(FPGA_RUNTIME_TRACE)' 'trace_hbm_bank=$(FPGA_TRACE_HBM_BANK)' \
		'trace_buffer_bytes=$(FPGA_TRACE_BUFFER_BYTES)' 'trace_max_records=$(FPGA_TRACE_MAX_RECORDS)' \
		'trace_cache_records=$(FPGA_TRACE_CACHE_RECORDS)' \
...
$(FPGA_SYNTH_DONE): FORCE $(FPGA_SYNTH_PREREQUISITES)
	...
	if test "$(FPGA_TYPE)" = zynqmp; then \
		...
		vivado -mode batch -nojournal -nolog -source "$(FPGA_SYNTH_TCL)" -tclargs \
			...
			"$(FPGA_VIVADO_SYNTH_JOBS)" "$(FPGA_CLOCK_MHZ)" "$(FPGA_RUNTIME_TRACE)"; \
	fi; \
