...
spmv-cuperflow-fpga-check:
	...
	@test "$(HOST_ABI)" = none && test "$(ACCELERATOR_HOST_ABI)" = spmv-cuperflow-u55c-v1
	@test "$(PROTOCOL_ABI)" = spmv-cuperflow-u55c-v1 && test "$(SPMV_CUPERFLOW_XRT_KERNEL)" = SpmvCuperflowKernel
	...
	@test "$(SPMV_CUPERFLOW_X_MEMORY_DATA_WIDTH)" = 256 && test "$(SPMV_CUPERFLOW_X_DECODER_LANES)" = 8
...
$(SPMV_CUPERFLOW_ELAB_DONE): FORCE spmv-cuperflow-fpga-check
	...
	"$(SPMV_CUPERFLOW_MANIFEST_TOOL)" verify "$(SPMV_CUPERFLOW_PARAMETER_MANIFEST)" \
		...
		SPMV_CUPERFLOW_X_DECODER_LANES=$(SPMV_CUPERFLOW_X_DECODER_LANES) \
