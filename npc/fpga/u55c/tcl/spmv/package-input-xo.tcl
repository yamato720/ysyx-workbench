...
proc load_source_manifest {manifest} {
  ...
  foreach line [split $content "\n"] {
    ...
    if {[string match "MODEL=*" $line]} { error "synthesis manifest contains a simulation model: $line" }
  ...
  if {!$synthesis_mode || [llength [dict get $result rtl]] == 0 || [llength [dict get $result xci]] != 1} {
    error "SPMV input synthesis manifest must contain RTL and exactly one FP64 XCI"
  }
  return $result
}

create_project spmv_input_kernel $project_dir -part $part -force
...
foreach {register_name register_offset register_access} {
  ...
  ctrl_beats 0x0b0 read-write
  batch_index 0x220 read-write
  product_checksum 0x230 read-only
  product_checksum_hi 0x234 read-only
  status 0x238 read-only
  cycle_counter 0x23c read-only
  cycle_counter_hi 0x240 read-only
} {
