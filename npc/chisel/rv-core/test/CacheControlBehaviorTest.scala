...
class CacheControlBehaviorTest extends AnyFlatSpec {
  ...
  "PipelinedIFetchAXIAdapter" should "issue a redirect target before stale responses drain" in {
    simulate(new PipelinedIFetchAXIAdapter(addrWidth = 32, dataWidth = 32, outstandingDepth = 4)) { dut =>
      ...
      // 先发出一个顺序取指请求，使 redirect 时 FIFO 中仍有旧 epoch 的响应。
      dut.io.axi.ar.valid.expect(true.B)
      ...
      // 新目标无需等待旧 R，立即在 FIFO 尾部发起 AR；旧 R 仍会在其之前按序返回。
      dut.io.axi.ar.valid.expect(true.B)
      ...
      dut.io.axi.r.bits.data.poke("h22222213".U)
      dut.io.responseValid.expect(true.B)
      dut.io.responsePc.expect(0x80000100L.U)
      dut.io.inst.expect("h22222213".U)
      dut.clock.step()
    }
  }
  ...
  it should "redirect the next AR to a static prediction while preserving the branch response" in {
    simulate(new PipelinedIFetchAXIAdapter(addrWidth = 32, dataWidth = 32, outstandingDepth = 4)) { dut =>
      ...
      dut.io.axi.r.bits.data.poke("h0000006f".U)
      dut.io.axi.r.valid.poke(true)
