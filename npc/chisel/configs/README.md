...
| 构造配方 | `common/core/`、`nemu/core/` | 可直挂终端 trait、检查 trait、NEMU host 与 FPGA 工具链 case class；不定义硬件 ABI | 所有 Make 终端必需 | `TerminalTraits.scala`、`CheckTraits.scala`、`NemuHostConfig.scala`、`FpgaToolchainConfig.scala` |
...
每个硬件领域都遵守 `base -> core -> Configs.scala` 的单向依赖：
...
- 领域根部的 `Configs.scala` 是唯一终端文件，只包含公共无参终端类。每个类只挂载一个 core 终端
  trait，由它完整提供运行构造、作用域和目标，再由终端提供所需 NEMU/FPGA 配方；终端不得直接混入
  base trait，检查构造和可复用成品也不得放入此文件。
- `common/`、`nemu/` 没有硬件终端，因此只保留 `base/`、`core/` 和必要的非 Config 工具入口，
  不创建空的 `Configs.scala`。FPGA 板卡共享 `fpga/common/base/`，各自在板卡目录的 `core/` 形成物理
  策略，并由同目录根部 `Configs.scala` 生成最终终端。

目录生成器会拒绝缺少根部 `Configs.scala` 的终端领域、出现在其他文件中的终端 trait、
`Configs.scala` 中未挂载 core 终端 trait 的 Config 类，以及终端对 base 构造 trait 的直接混入。
...
所有本地 NPC/SoC 仿真终端都通过 `NpcTerminal` 或 `SocTerminal` 提供本地构造行为，并直挂
...
`common/base/ConstructionTraits.scala` 中的底层接口，只供 core 组合，终端不能直接混入。
`NpcTerminal`、`SocTerminal`、`FpgaNpcTerminal`、`FpgaSocTerminal` 是
`common/core/TerminalTraits.scala` 中仅有的四种 Make 终端 trait；每个终端只挂载其中一个。
本地 trait 要求 local NEMU 配方，FPGA trait 同时要求板卡匹配的 NEMU 与 FPGA 工具链配方。
工具链按 `device`、`flow`、`reports`、`runtime` 分组，可用
`U55cBase.copy(flow = U55cBase.flow.copy(...))` 局部覆盖。`CheckOnlyConstruction` 是检查构造直接挂载的
