...
- `base/` 只定义参数键、普通数据模型、组合协议和 `With...Config` 原子片段。它不混入终端 marker，
  不引用 `core/`，也不直接表达某个可运行目标。
- `core/` 调用 `base/`，把 ISA、流水线、接口、内存、SoC 或板卡策略组合成名称直观、含义完整的
  成品。终端必须直接引用这些成品，不能在终端文件里重新展开底层片段。
- 领域根部的 `Configs.scala` 是唯一终端文件，只包含公共无参终端类。每个类都挂载一种运行构造
  trait、一个作用域/目标 marker，以及所需 NEMU/FPGA 配方；检查构造和可复用成品不得放入此文件。
...
目录生成器会拒绝缺少根部 `Configs.scala` 的终端领域、出现在其他文件中的终端 marker，以及
`Configs.scala` 中未带 marker 的 Config 类。移动文件时保持原 package，避免无意改变公开 FQCN。
