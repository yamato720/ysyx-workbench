...
| 终端 trait | `common/TerminalTraits.scala` | 组合底层运行行为、目录身份、scope 和 target，不承载硬件组合 | 所有 Make 终端必需 | 根部直挂文件 |
...
- 终端级文件与 `base/`、`core/` 文件夹分离，直接位于领域根部。当前四种共享终端 trait 统一位于
...
`common/TerminalTraits.scala` 中仅有的四种 Make 终端 trait；每个终端只挂载其中一个。
