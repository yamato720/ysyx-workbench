...
| --- | --- | --- | --- | --- |
| 公共 IP 描述 | `common/` | 不绑定 NPC、SoC 或板卡，供 CPU、外设和加速器复用 | 始终编译，不独立生成 | 无；仅通用 IP 参数数据 |
| 构造配方 | `common/`、`nemu/` | 两类运行 trait、NEMU host 与 FPGA 工具链 case class；不定义硬件 ABI | 所有 Make 终端必需 | `HostConstructionConfigs.scala`、`NemuConfigs.scala`、`FpgaToolchainConfig.scala` |
| L1 NPC | `npc/` | 仅依赖 CDE 参数库，不依赖 Rocket/板卡 | 必需 | `Configs.scala` |
| L2 SoC | `ysyx/` | 依赖 L1 与 Rocket CDE | SoC 才需要 | `Configs.scala` |
| L3 FPGA | `fpga/common/` | 把 L1/L2 接入 FPGA CDE | FPGA 才需要 | `core/` 与 `core/dependencies/` |
