...
| 终端 trait | `common/TerminalTraits.scala` | 闭合 NEMU/FPGA 配方、底层运行行为、目录身份、scope 和 target，不承载硬件组合 | 所有 Make 终端必需 | 根部直挂文件 |
...
- 领域根部的 `Configs.scala` 是唯一终端 Config 文件，只包含公共无参终端类。每个类只挂载一个 terminal 层
  预设，由它完整提供运行构造、作用域、目标和 NEMU/FPGA 配方；终端不得手动重载配方或直接混入
...
所有本地 NPC/SoC 仿真终端都通过 `LocalNpcTerminal` 或 `LocalSocTerminal` 闭合本地构造行为与
...
预设；每个终端只挂载其中一个。工具链按 `device`、`flow`、`reports`、`runtime` 分组。确需新配方时，
先在 `core/` 用 `copy(...)` 定义具名完整 case class 预设，再在根部定义闭合 terminal trait，并同步
目录注册、解析校验和测试；最终 `Configs.scala` 不允许局部覆盖。`CheckOnlyConstruction` 是检查构造直接
挂载的 core trait，不进入 Make 目录。公共构造 trait 名称不使用 `Trait` 后缀，承载这些 trait 的文件
统一使用 `*Traits.scala`。自动目录不再从类名后缀猜测目标；所有硬件参数应在可追踪的 Scala 组合中固定。
