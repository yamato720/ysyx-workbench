...
class BaseIsaConfig extends ConfigFragment {
  override private[npc] def applyTo(base: NpcConfig): NpcConfig = base.copy(
    isa = base.isa.copy(M = false, F = false, D = false, Zicsr = false)
  )
}
...
class WithFloatingPointConfig extends ConfigFragment {
  override private[npc] def applyTo(base: NpcConfig): NpcConfig = {
    ...
    base.copy(
      ...
      operators = base.operators.copy(routes =
        base.operators.routes.fillMissing(OperatorRouteConfig.modelF(base.isa.xlen, base.operators.floating)))
    )
  }
}
...
class WithoutFloatingPointConfig extends ConfigFragment {
  override private[npc] def applyTo(base: NpcConfig): NpcConfig =
    base.copy(isa = base.isa.copy(F = false, D = false))
}
