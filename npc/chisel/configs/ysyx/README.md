...
| 文件 | 职责 | 是否是上层可复用成品 |
| --- | --- | --- |
| `core/YsyxBaseConfig.scala` | Rocket RV32 与边缘总线基础层 | `BaseYsyxConfig` 可复用，但通常经 SoC 成品间接使用 |
| `core/dependencies/YsyxParameters.scala` | ysyxSoC 平台、ChipLink、AXI SDRAM 的 CDE 键和查询接口 | 否；供 L2/L3 读取参数 |
| `Configs.scala` | 默认 NPC、可复用 SoC 图与本地仿真终端 | 是；`YsyxSocConfig` 是 L2 基类 |
...
| --- | --- | --- | --- |
| Rocket RV32/边缘总线基础 | `new BaseYsyxConfig` | `core/YsyxBaseConfig.scala` | L2 构造必需 |
| 默认 SoC | `new YsyxSocConfig` | `Configs.scala` | L2 构造必需 |
| 可复用 SoC 图 | `new YsyxElaborateConfig` | `Configs.scala` | 是；供板卡或直接 Scala elaboration 叠加，不是 Make 终端 |
| 本地仿真 SoC 终端 | `new YsyxSimulationConfig` | `Configs.scala` | 是；未绑定 FPGA 板卡时使用本地 Verilator host |
| 平台参数查询 | `YsyxPlatformParameters.isDpiSimulation`、`YsyxPlatformParameters.isFpga` | `core/dependencies/YsyxParameters.scala` | 模块内部使用 |
