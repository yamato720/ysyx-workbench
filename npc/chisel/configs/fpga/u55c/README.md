...
| `npc/Configs.scala` | U55C 裸 NPC、RV64IM 与性能监测终端 | 是；package 为 `npc.fpga.u55c` |
| `ysyx/Configs.scala` | U55C ysyxSoC 终端 | 是；package 为 `ysyx.fpga.u55c` |
| `spmv/Configs.scala` | U55C SPMV 资产终端 | 是；package 为 `accelerators.spmv.fpga.u55c` |
...
的默认 NPC；板卡键自动选择 SoC 的 FPGA 分支。所有完整终端按产品目录发现，使用统一的
`fpga` 作用域，并由 `TARGET=NPC|SOC|SPMV` 选择对应生成入口。
...
| U55C 裸 NPC 终端 | `new U55cNpcFpgaConfig` | `npc/Configs.scala` | 是 |
...
| U55C 板卡标识 | `new WithFpgaBoardConfig(FpgaBoard.U55c)` | L3 `base/FpgaConfigFragments.scala` | U55C 目标必需 |
