...
| part、引脚、DDR、XDC、vendor IP | 无；不在 L3 添加 | `npc/fpga/<board>/` 与 `npc/fpga-ip-generator/` | 不适用 |

L3 本身不提供可直接布线的物理顶层；选择 FPGA 后必须继续选择一个 L4 板卡终端 Config。
所有 FPGA Config 固定禁用 F/D 和精确指令 assist；浮点学习仅使用 `npc/Configs.scala` 中的本地仿真终端。
