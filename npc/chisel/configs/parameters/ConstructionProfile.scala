...
object ConstructionProfile {
  ...
  def values(
    ...
    val base = Seq(
      ...
      "BRANCH_PREDICTOR" -> bit(config.branchPredictor.enabled),
      "BRANCH_PREDICTOR_ENTRIES" -> config.branchPredictor.table.entries.toString,
      "BRANCH_PREDICTOR_RETURN_ENTRIES" -> config.branchPredictor.table.returnEntries.toString,
