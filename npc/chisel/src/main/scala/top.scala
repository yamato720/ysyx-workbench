...
class CPU(Width:Int = 64, Debug:Boolean = false, useDPI:Boolean = false,
  ...
  val io = IO(new Bundle{
    ...
    val addr_low = if(Debug) Some(Output(UInt(Width.W))) else None
    val addr_high = if(Debug) Some(Output(UInt(Width.W))) else None
    ...
    val regs_out = if (Debug) Some(Output(Vec(128, UInt(Width.W)))) else None
    val opcode_out = if(Debug) Some(Output(UInt(7.W))) else None
    val alu_result_out = if(Debug) Some(Output(UInt(Width.W))) else None
    val func3_out = if(Debug) Some(Output(UInt(3.W))) else None
    val func7_out = if(Debug) Some(Output(UInt(7.W))) else None
    val mem_result_out = if(Debug) Some(Output(UInt(Width.W))) else None
  ...
  DataMemory_inst.io.mem_write_en := OpcodeCtrlTop_inst.io.memWrite
  DataMemory_inst.io.mem_read_en := OpcodeCtrlTop_inst.io.memRead
  ...
  // Branch/Jump控制：当branch=false时sel=0(PC+4)，当branch=true时sel=branch_taken
  mux8_3_inst.io.sel := Mux(OpcodeCtrlTop_inst.io.branch, ALU_Top_inst.io.branch_taken, 0.U)
