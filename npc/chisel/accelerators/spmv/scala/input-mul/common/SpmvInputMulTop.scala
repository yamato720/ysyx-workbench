...
/** SPMV 输入层顶层接口。
  ...
  */
final class SpmvInputMulTopIO(config: SpmvInputConfig) extends Bundle {
...
private[inputmul] final class SpmvRequestCompletionTracker(streamCount: Int) extends Module {
...
/**
  ...
  * 填充和顶层状态端口。
  */
abstract class SpmvInputMulTop(
  config: SpmvInputConfig,
  expectedSchedule: SpmvXPortSchedule
) extends Module {
  require(config.xPortSchedule == expectedSchedule,
    s"输入乘法顶层模式为 ${expectedSchedule.profileName}，实际为 ${config.xPortSchedule.profileName}")
  ...
  val io = IO(new SpmvInputMulTopIO(config))
  ...
  consumers.zipWithIndex.foreach { case (consumer, index) =>
    ...
    mulEngine.io.clearChecksum := !io.mulEnable
  ...
  private val pingPongSchedule = expectedSchedule == SpmvXPortSchedule.PingPong
  ...
  localXs.zip(mulEngines).zipWithIndex.foreach { case ((localX, mulEngine), index) =>
    ...
    mulEngine.io.aSlotValidMask := "hff".U(config.fp64MultiplyLaneCount.W)
