...
import npc.{
  ...
  ConfigCatalog,
  MakeTerminal,
  SpmvInputSimulationConstruction,
  SpmvInputSimulationProfile,
  SpmvInputConfigKey,
}

/** 为独立 SPMV 输入 smoke 终端生成不含 CPU/NEMU 字段的 profile。 */
object DescribeSpmvInputSimulationConfig extends App {
  require(args.length == 1, "用法：accelerator.spmv.DescribeSpmvInputSimulationConfig <profile.env>")
  ...
  val simulation = construction match {
    ...
    case _ => throw new IllegalArgumentException(s"${entry.className} 未挂载 SPMV 输入 smoke 构造")
  }
  ...
  ConstructionProfile.write(
    Path.of(args(0)),
    SpmvInputSimulationProfile.values(entry, simulation, input)
  )
}
