...
import npc.{AcceleratorHostConfig, AcceleratorHostConstruction, ConfigCatalog, FpgaBitstreamConstruction, FpgaSynthesisConstruction, FpgaToolchainConfig}
...
class SpmvAcceleratorConfigTest extends AnyFlatSpec {
  ...
  "SpmvConstructionProfile" should "publish accelerator fields without CPU or NEMU fields" in {
    val entry = ConfigCatalog.Entry(
      ...
      className = "accelerators.spmv.fpga.u55c.ProbeConfig",
      scope = "fpga",
      board = Some("u55c"),
      target = "SPMV"
    )
  ...
  it should "publish the FP64 eight-lane bitstream ABI without CPU fields" in {
    val entry = ConfigCatalog.Entry(
      ...
      className = "accelerators.spmv.fpga.u55c.BitstreamProbeConfig",
      scope = "fpga",
      board = Some("u55c"),
      target = "SPMV"
    )
