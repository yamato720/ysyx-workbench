...
/** 带固定粒度写掩码的真双口片上 RAM。
  *
  * Cuperflow 的 512-bit X line 以 64-bit 为粒度更新。连续 X 一次写满整行；灵活 X
  * 的 marker 可以使一拍中的 value 落到不同 line，因此需要保留未触及的 64-bit lane。
  */
final class OnChipMaskedMemoryPortIO(
  val addrWidth: Int,
  val dataWidth: Int,
  val writeMaskWidth: Int
) extends Bundle {
  val enable = Input(Bool())
  val write = Input(Bool())
  val address = Input(UInt(addrWidth.W))
  val wdata = Input(UInt(dataWidth.W))
  val wmask = Input(UInt(writeMaskWidth.W))
  val rdata = Output(UInt(dataWidth.W))
}

/** 两口均支持掩码写的公共片上 RAM。
  *
  * `writeMaskWidth` 必须整除数据位宽。综合时映射为 XPM 的 byte-write interface；在
  * Cuperflow 中每一个 write-enable 对应一个 64-bit FP64 lane，而不是 8-bit byte。
  */
final class OnChipMaskedTrueDualPortMemory(
  val depth: Int,
  val dataWidth: Int,
  val writeMaskWidth: Int,
  val primitive: OnChipMemoryPrimitive = OnChipMemoryPrimitive.UltraRam
  ...
  require(dataWidth >= 8 && (dataWidth & (dataWidth - 1)) == 0,
    s"片上 RAM 数据位宽必须是至少 8 bit 的二次幂，实际为 $dataWidth")
  require(writeMaskWidth > 0 && dataWidth % writeMaskWidth == 0,
    s"掩码宽度必须整除数据宽度，实际为 $writeMaskWidth/$dataWidth")
  require((dataWidth / writeMaskWidth) % 8 == 0,
    s"每个写掩码 lane 必须包含整数个 byte，实际为 ${dataWidth / writeMaskWidth} bit")

  private val addrWidth = log2Ceil(depth)
  val io = IO(new Bundle {
    val a = new OnChipMaskedMemoryPortIO(addrWidth, dataWidth, writeMaskWidth)
    val b = new OnChipMaskedMemoryPortIO(addrWidth, dataWidth, writeMaskWidth)
  })

  private val impl = Module(new OnChipMaskedTrueDualPortMemoryBlackBox(
    depth, dataWidth, addrWidth, writeMaskWidth, primitive.profileName))
  impl.io.clock := clock
  impl.io.a_enable := io.a.enable
  impl.io.a_write := io.a.write
  impl.io.a_address := io.a.address
  impl.io.a_wdata := io.a.wdata
  impl.io.a_wmask := io.a.wmask
  io.a.rdata := impl.io.a_rdata
  impl.io.b_enable := io.b.enable
  impl.io.b_write := io.b.write
  impl.io.b_address := io.b.address
  impl.io.b_wdata := io.b.wdata
  impl.io.b_wmask := io.b.wmask
  io.b.rdata := impl.io.b_rdata
}

private final class OnChipMaskedTrueDualPortMemoryBlackBox(
  depth: Int,
  dataWidth: Int,
  addrWidth: Int,
  writeMaskWidth: Int,
  ...
  "WRITE_MASK_WIDTH" -> writeMaskWidth,
  "PRIMITIVE" -> primitive
  ...
  val io = IO(new Bundle {
    ...
    val a_wmask = Input(UInt(writeMaskWidth.W))
    ...
    val b_wmask = Input(UInt(writeMaskWidth.W))
    val b_rdata = Output(UInt(dataWidth.W))
  })

  override def desiredName: String = "NpcOnChipMaskedTrueDualPortMemory"

  setInline("NpcOnChipMaskedTrueDualPortMemory.sv",
