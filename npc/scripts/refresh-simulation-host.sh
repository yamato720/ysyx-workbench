...
NEMU_MEMORY_STATISTICS_MODE=${NEMU_MEMORY_STATISTICS_MODE:-Split}
case "$NEMU_MEMORY_STATISTICS_MODE" in
  Split|ServiceOnly) ;;
  *) echo "NEMU_MEMORY_STATISTICS_MODE 必须为 Split 或 ServiceOnly" >&2; exit 2 ;;
esac
...

{
  ...
  echo "NEMU_MEMORY_STATISTICS_MODE=$NEMU_MEMORY_STATISTICS_MODE"
  echo "SCOPE=$SCOPE"
  echo "TARGET=$TARGET"
  echo "XLEN=$XLEN"
  echo "F=$F"
