...
- Staging constructions carry `.incomplete`; after host/RTL/asset validation the manager writes `.complete` and only then atomically publishes the directory. `make -C npc version` reads complete published metadata without waiting for a long-running build lock, and refreshes the catalog only when a Scala Config is newer than the saved catalog. FPGA completion additionally requires the U55C xclbin or ZCU102 `npc.bit` to exist and be nonempty; full manifest hashes remain a build/run validation. The query only waits when an old construction still needs its one-time version metadata migration. `resolve` uses the same nonblocking published snapshot for saved versions but may refresh a current Config profile when no saved version was selected.
- Read the `version` output as three tables: the primary table uses `+`/blank feature cells for RV32/RV64, M/F/Zicsr, Pipe/ID/EX, target, and FPGA presence only; the second maps saved version numbers to Config short names; the third lists every current public Config from the catalog, including unbuilt FPGA terminals. `D=<positive version>` deletes a saved construction and compacts later version numbers. Pipeline always occupies exactly the Pipe, ID-forwarding, and EX-forwarding cells.
